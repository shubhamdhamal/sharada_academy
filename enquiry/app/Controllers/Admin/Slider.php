...
class Slider extends App_Controller {
    ...
    public function order() {
        ...
        $page_data['page_js'] = $page_js;
        $page_data['slider'] = $this->Crud_model->get_data($this->table, array('is_active'=>'1'), array(), array('order'=>'asc'));
        return admin_view('index',$page_data);
    }
            ...
            if($action=='list'){
                ...
                if ( isset($datalist) && ! empty($datalist)) {
                    foreach ($datalist as $list) {
                        $edit = '<a href="'.admin_site_url("slider/page/add-edit/".encode_string($list->id)).'" class="btn ripple btn-primary btn-sm mt-1 mb-1 text-sm text-white popup-page" data-placement="top" data-toggle="tooltip" title="'.translate('details').'"> <i class="fa fa-edit"></i> </a>';
                        $details = '<a href="'.admin_site_url("slider/page/details/".encode_string($list->id)).'" class="btn ripple btn-secondary btn-sm mt-1 mb-1 text-sm text-white popup-page" data-placement="top" data-toggle="tooltip" title="'.translate('details').'"> <i class="fa fa-eye"></i> </a>';
                        $status = $list->is_active=='1' ? '<span class="badge bg-success cursor-pointer change-slider" data-action="change_slider" data-id="'.encode_string($list->id).'" data-url="'.admin_site_url('slider/crud').'">'.translate('active').'</span>' : '<span class="badge bg-danger cursor-pointer change-slider" data-action="change_slider" data-id="'.encode_string($list->id).'" data-url="'.admin_site_url('slider/crud').'">'.translate('inactive').'</span>';
                        ...
                        $nestedData['actions']      = '<div class="btn-group" slider="group"">'.$edit.$details.'</div>';
                        $data[]                     = $nestedData;
                    }
                }
                $json_data = [
                ...
                $validation['is_active'] = ['label' => translate('slider'), 'rules' => 'required'];
                ...
                $id = $this->request->getPost('id');
                $id = decode_string($id);
                $slider = $this->Crud_model->get_data_row($this->table,array('id'=>$id));
                if(!empty($slider)){
                    $up_data = array();
                    $up_data['is_active'] = '1';
                ...
                if($this->Crud_model->status_trans()){
                    $new_data = $this->Crud_model->get_data_row($this->table,array('id'=>$id));
                    add_app_log($this->table,'7',$id,$new_data,$slider);
                    
                    $json['status'] = true;
                    $json['type'] = 'success';
                    $json['message'] = translate('slider_update_successfully');
                }else{
                    $json['message'] = translate('unable_to_update_slider');
                }
                ...
                if($this->Crud_model->status_trans()){
                    ...
                    $json['url'] = admin_site_url('slider');
                }else{
                    $json['message'] = translate('something_went_wrong');
                }
            }
    ...
    function page($slug='',$id=''){
        if ($this->request->isAJAX()) {
            ...
            if($slug=='add-edit'){
                $page_data = array();
                $page_data['page_title'] = $id!='' ? translate('update_slider') : translate('new_slider');
                $page_data['page_action'] = $id!='' ? 'update_slider' : 'new_slider';
                $page_data['id'] = $id;
                echo admin_view($this->module_name.'/popup-add-edit',$page_data);
            }else if($slug=='details'){
                $page_data = array();
                $page_data['page_title'] = translate('slider_details');
                $page_data['id'] = $id;
...
/* Location: ./app/controllers/Slider.php */