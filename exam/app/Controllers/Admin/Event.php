...
class Event extends App_Controller {
    private $module_name = 'event';
    private $table = TBL_EVENT;
    private $module_title;
    function __construct() {
        parent::__construct();
        $this->module_title = translate('event');
    }
    ...
    public function order() {
        ...
        $page_data['page_js'] = $page_js;
        $page_data['event'] = $this->Crud_model->get_data($this->table, array('is_active'=>'1'), array(), array('order'=>'asc'));
        return admin_view('index',$page_data);
    }
            ...
            if($action=='list'){
                ...
                if ( isset($datalist) && ! empty($datalist)) {
                    foreach ($datalist as $list) {
                        $edit = '<a href="'.admin_site_url("event/page/add-edit/".encode_string($list->id)).'" class="btn ripple btn-primary btn-sm mt-1 mb-1 text-sm text-white popup-page" data-placement="top" data-toggle="tooltip" title="'.translate('details').'"> <i class="fa fa-edit"></i> </a>';
                        $details = '<a href="'.admin_site_url("event/page/details/".encode_string($list->id)).'" class="btn ripple btn-secondary btn-sm mt-1 mb-1 text-sm text-white popup-page" data-placement="top" data-toggle="tooltip" title="'.translate('details').'"> <i class="fa fa-eye"></i> </a>';
                        $status = $list->is_active=='1' ? '<span class="badge bg-success cursor-pointer change-event" data-action="change_event" data-id="'.encode_string($list->id).'" data-url="'.admin_site_url('event/crud').'">'.translate('active').'</span>' : '<span class="badge bg-danger cursor-pointer change-event" data-action="change_event" data-id="'.encode_string($list->id).'" data-url="'.admin_site_url('event/crud').'">'.translate('inactive').'</span>';
                        ...
                        $nestedData['date']         = utc_to_local_datetime($list->date,'date');
                        $nestedData['created_on']   = utc_to_local_datetime($list->created_on);
                        $nestedData['status']       = $status;
                        $nestedData['actions']      = '<div class="btn-group" event="group"">'.$edit.$details.'</div>';
                        $data[]                     = $nestedData;
                    }
                }
                $json_data = [
                ...
                $validation['date'] = ['label' => translate('date'), 'rules' => 'required'];
                $validation['is_active'] = ['label' => translate('event'), 'rules' => 'required'];
                ...
                $uis = array('name','date','short_description','url','is_active');
                $up_data = array();
                foreach ($uis as $key) {
                ...
                $id = $this->request->getPost('id');
                $id = decode_string($id);
                $event = $this->Crud_model->get_data_row($this->table,array('id'=>$id));
                if(!empty($event)){
                    $up_data = array();
                    $up_data['is_active'] = '1';
                ...
                if($this->Crud_model->status_trans()){
                    $new_data = $this->Crud_model->get_data_row($this->table,array('id'=>$id));
                    add_app_log($this->table,'7',$id,$new_data,$event);
                    
                    $json['status'] = true;
                    $json['type'] = 'success';
                    $json['message'] = translate('event_update_successfully');
                }else{
                    $json['message'] = translate('unable_to_update_event');
                }
    ...
    function page($slug='',$id=''){
        if ($this->request->isAJAX()) {
            ...
            if($slug=='add-edit'){
                $page_data = array();
                $page_data['page_title'] = $id!='' ? translate('update_event') : translate('new_event');
                $page_data['page_action'] = $id!='' ? 'update_event' : 'new_event';
                $page_data['id'] = $id;
                echo admin_view($this->module_name.'/popup-add-edit',$page_data);
            }else if($slug=='details'){
                $page_data = array();
                $page_data['page_title'] = translate('event_details');
                $page_data['id'] = $id;
...
/* Location: ./app/controllers/Event.php */