...
class Testimonial extends App_Controller {
    private $module_name = 'testimonial';
    private $table = TBL_TESTIMONIAL;
    private $module_title;
    function __construct() {
        parent::__construct();
        $this->module_title = translate('testimonial');
    }
    ...
    public function add() {
        ...
        $page_data['page_title'] = 'add_new_testimonial';
        $page_data['page_action'] = 'new_testimonial';
        $page_data['page_js'] = $page_js;
        $page_data['id'] = '';
        return admin_view('index',$page_data);
    }
    public function edit($id) {
        ...
        $page_data['module_name'] = $this->module_name;
        $page_data['page_name'] = 'add-edit';
        $page_data['page_title'] = 'update_testimonial';
        $page_data['page_action'] = 'update_testimonial';
        $page_data['page_js'] = $page_js;
    ...
    public function order() {
        ...
        $page_data['page_js'] = $page_js;
        $page_data['testimonial'] = $this->Crud_model->get_data($this->table, array('is_active'=>'1'), array(), array('order'=>'asc'));
        return admin_view('index',$page_data);
    }
        ...
        if ($this->request->isAJAX()) {
            ...
            if($action=='list'){
                ...
                if(isset($search['value']) && $search['value']!=''){
                    $like = array($search['value'],'name,testimonialname,email_id,mobile_no');
                }
                ...
                if ( isset($datalist) && ! empty($datalist)) {
                    foreach ($datalist as $list) {
                        ...
                        if(valid_session($this->module_name,'edit',false)){
                            $edit = '<a href="'.admin_site_url("testimonial/edit/".encode_string($list->id)).'" class="btn ripple btn-primary btn-sm mt-1 mb-1 text-sm text-white" data-placement="top" data-toggle="tooltip" title="'.translate('edit').'"> <i class="fa fa-edit"></i> </a>';
                        }
                        $details = '<a href="'.admin_site_url("testimonial/page/details/".encode_string($list->id)).'" class="btn ripple btn-secondary btn-sm mt-1 mb-1 text-sm text-white popup-page" data-placement="top" data-toggle="tooltip" title="'.translate('details').'"> <i class="fa fa-eye"></i> </a>';
                        $status = $list->is_active=='1' ? '<span class="badge bg-success cursor-pointer change-status" data-action="change_status" data-id="'.encode_string($list->id).'" data-url="'.admin_site_url('testimonial/crud').'">'.translate('active').'</span>' : '<span class="badge bg-danger cursor-pointer change-status" data-action="change_status" data-id="'.encode_string($list->id).'" data-url="'.admin_site_url('testimonial/crud').'">'.translate('inactive').'</span>';
                ...
                $uis = array('name','short_description','position','is_active');
                $up_data = array();
                foreach ($uis as $key) {
                ...
                if($this->Crud_model->status_trans()){
                    ...
                    $json['url'] = admin_site_url('testimonial');
                }else{
                ...
                $id = $this->request->getPost('id');
                $id = decode_string($id);
                $testimonial = $this->Crud_model->get_data_row($this->table,array('id'=>$id));
                ...
                if($this->Crud_model->status_trans()){
                    $new_data = $this->Crud_model->get_data_row($this->table,array('id'=>$id));
                    add_app_log($this->table,'7',$id,$new_data,$testimonial);
    ...
    function page($slug='',$id=''){
        if ($this->request->isAJAX()) {
            ...
            if($slug=='details'){
                $page_data = array();
                $page_data['page_title'] = translate('testimonial_details');
                $page_data['id'] = $id;
...
/* Location: ./app/controllers/testimonial.php */