			...
			<div class="row text-center pl-0 pr-0 ml-0 mr-0">
				<div class="col-lg-4 d-block mx-auto">
					...
					<div class="card custom-card">
						<div class="card-body">
							<h4 class="text-center"><?php echo translate('forgot_your_account_password'); ?></h4>
							<?php echo form_open(admin_site_url('auth/crud'), array('class'=>'data-parsley-validate auth-forgot-form mt-2', 'method'=>'post'), array('action'=>'forgot')); ?>
								<div class="form-group text-left">
	                                <label><?php echo translate('username'); ?> <span class="text-danger">*</span></label>
	                                <input type="text" class="form-control" name="username" placeholder="<?php echo translate('username'); ?>" tabindex="1" autocomplete="off" autofocus required />
	                            </div>
	                            <?php if(app_setting('captcha_status','off')!='off'){ ?>
                                    ...
                                    <?php if($captcha_type=='default'){ ?>
                                        <div class="form-group text-left">
                                            ...
                                            <div class="d-flex">
                                                <input type="text" class="form-control" id="txt_forgot_captcha" name="captcha_token" placeholder="<?php echo translate('captcha'); ?>" tabindex="3" data-parsley-errors-container="#error_captcha" autocomplete="off" required />
                                                <img src="<?php echo get_captcha('default', 'forgot_captcha')->img; ?>" id="forgot_captcha" class="img-responsive cursor-pointer referesh-captcha" alt="<?php echo translate('default_captcha') ?>" data-toggle="tooltip" data-placement="top" title="<?php echo translate('click_to_regenerate_new_captcha'); ?>">
                                            </div>
                                            <span id="error_captcha"></span>
                                        </div>
                                    <?php }else if($captcha_type=='grcv2'){ ?>
                                        <div class="form-group text-left">
                                            <?php echo get_captcha($captcha_type, 'forgot_captcha'); ?>
                                            <script src='https://www.google.com/recaptcha/api.js?onload=onloadCallback&render=explicit' async defer></script>    
                                            <script>
                                                var onloadCallback = function() {
                                                    var grcid = 'forgot_captcha';
                                    ...
                                    <?php } ?>
                                <?php } ?>
								<button type="submit" class="btn ripple btn-main-primary btn-block" tabindex="4" data-loading-text="<span aria-hidden='true' class='spinner-border spinner-border-sm'></span> <?php echo translate('please_wait...'); ?>"><?php echo translate('send_instructions'); ?></button>
							<?php echo form_close(); ?>
							<div class="mt-3 text-center">
								<p class="mb-0"><a href="<?php echo admin_site_url('auth'); ?>"><?php echo translate('remember_password'); ?>?</a></p>
							</div>
						</div>
		...
		<?php if(app_setting('captcha_status','off')!='off'){ ?>
            ...
            <script>
                function regenerate_captcha(){
                    <?php if($captcha_type=='default'){ ?>
                        $.post('<?php echo admin_site_url('auth/generate-captcha') ?>',{action:'captcha', type:'forgot_captcha'},function(data) {
                            if(data.status){
                                $("#forgot_captcha").attr('src',data.details);
                                $("#txt_forgot_captcha").val('');
                            }
                        });
                    <?php }else if($captcha_type=='grcv2'){ ?>
                        var grcid = 'forgot_captcha';
                    ...
                    <?php }else if($captcha_type=='grcv3'){ ?>
                	    grecaptcha.ready(function() {
	                        grecaptcha.execute('<?php echo $details->captcha_site_key ?>', {action: 'forgot'}).then(function(token) {
