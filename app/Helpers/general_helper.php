...
if(!function_exists('get_filemanager_key')){
    function get_filemanager_key($uid,$web_auth_key=''){
        if($web_auth_key!=''){
            $web_auth_key = get_column_value(TBL_ADMIN,array('id'=>$uid),'web_auth_key');
        }
        return sha1($web_auth_key.'.'.$uid.'DMITPL');
    }
}
...
if(!function_exists('app_file_manager')){
    function app_file_manager($name,$id,$type=1,$default='',$required=true){ //0 = Filemanager,1 = Image,2 = File,3 = Video
        ...
        $akey = get_filemanager_key($user_id);
        
        $req = $required==true ? 'required' : '';
        if($type==1){
            $default = $default!='' ? $default : 'default.png';
            $html = '<a href="javascript:void(0);" data-url="'.base_url('filemanager/dialog.php?akey='.$akey.'&type='.$type.'&field_id='.$id).'" class="btn-iframe" data-original-title="'.translate('click_on_the_image_to_change').'" data-placement="top" data-toggle="tooltip" title="'.translate('click_on_the_image_to_change').'">
            ...
            $html = '<div class="input-group">
                    ...
                    <div class="input-group-btn">
                        <a class="btn ripple btn-primary rounded-start-0 btn-iframe" href="javascript:void(0);" data-url="'.base_url('filemanager/dialog.php?akey='.$akey.'&type='.$type.'&field_id='.$id).'"><i class="fa fa-folder-open-o"></i></a>
                    </div>
                </div>
...
if(!function_exists("get_file_download_url")){
    function get_file_download_url($file,$is_home=false){
        if(!empty($file)){
            $path = WRITEPATH.'files\\'.$file->path;
            $pathinfo = pathinfo($path);
            $token = array();
            $token['path'] = $pathinfo['filename'];
            $token['name'] = $file->name;
            $token['extension'] = $pathinfo['extension'];
            $token['token'] = session('web_auth_key');
            
            if($is_home){
                $url = home_site_url('dl').'?file='.encode_string(json_encode($token));
            }else{
                $url = admin_site_url('dl').'?file='.encode_string(json_encode($token));
            }
            return $url;
        }
    }
