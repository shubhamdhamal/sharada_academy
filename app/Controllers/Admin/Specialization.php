...
class Specialization extends App_Controller {
    private $module_name = 'specialization';
    private $table = TBL_SPECIALISATION;
    private $module_title;
    function __construct() {
        parent::__construct();
        $this->module_title = translate('specialization');
    }
    public function index() {
        ...
        $page_data['page_title'] = 'specialization';
    ...
    public function order() {
        ...
        $page_data['page_js'] = $page_js;
        $page_data['specialization'] = $this->Crud_model->get_data($this->table, array('is_active'=>'1'), array(), array('order'=>'asc'));
        return admin_view('index',$page_data);
    }
    public function crud(){
        ...
        if ($this->request->isAJAX()) {
            ...
            if($action=='list'){
                ...
                if ( isset($datalist) && ! empty($datalist)) {
                    foreach ($datalist as $list) {
                        $edit = '<a href="'.admin_site_url("specialization/page/add-edit/".encode_string($list->id)).'" class="btn ripple btn-primary btn-sm mt-1 mb-1 text-sm text-white popup-page" data-placement="top" data-toggle="tooltip" title="'.translate('details').'"> <i class="fa fa-edit"></i> </a>';
                        $details = '<a href="'.admin_site_url("specialization/page/details/".encode_string($list->id)).'" class="btn ripple btn-secondary btn-sm mt-1 mb-1 text-sm text-white popup-page" data-placement="top" data-toggle="tooltip" title="'.translate('details').'"> <i class="fa fa-eye"></i> </a>';
                        $status = $list->is_active=='1' ? '<span class="badge bg-success cursor-pointer change-status" data-action="change_status" data-id="'.encode_string($list->id).'" data-url="'.admin_site_url('specialization/crud').'">'.translate('active').'</span>' : '<span class="badge bg-danger cursor-pointer change-status" data-action="change_status" data-id="'.encode_string($list->id).'" data-url="'.admin_site_url('specialization/crud').'">'.translate('inactive').'</span>';
                        ...
                        $nestedData['actions']      = '<div class="btn-group" specialization="group"">'.$edit.$details.'</div>';
                        $data[]                     = $nestedData;
                    }
                }
                $json_data = [
            ...
            }else if($action=='new_specialization'){
            ...
            }else if($action=='update_specialization'){
                ...
                $specialization = $this->Crud_model->get_data_row($this->table,array('id'=>$id));
                if(!empty($specialization)){
                    $up_data = array();
                    $up_data['is_active'] = '1';
                    if($specialization->is_active=='1'){
                        $up_data['is_active'] = '0';
                    }
                    $up_data['updated_by'] = json_encode($updated);
                    $up_data['updated_on'] = date(DB_DATETIME_FORMAT);
                    if($this->Crud_model->update_data($this->table,$up_data,array('id'=>$id))){
                        $new_data = $this->Crud_model->get_data_row($this->table,array('id'=>$id));
                        add_app_log($this->table,'7',$id,$new_data,$specialization);
                ...
                if($this->Crud_model->status_trans()){
                    ...
                    $json['url'] = admin_site_url('specialization');
                }else{
                    $json['message'] = translate('something_went_wrong');
                }
            }
    ...
    function page($slug='',$id=''){
        ...
        if ($this->request->isAJAX()) {
            if($slug=='add-edit'){
                $page_data = array();
                $page_data['page_title'] = $id!='' ? translate('update_specialization') : translate('new_specialization');
                $page_data['page_action'] = $id!='' ? 'update_specialization' : 'new_specialization';
                $page_data['id'] = $id;
                echo admin_view($this->module_name.'/popup-add-edit',$page_data);
            }else if($slug=='details'){
                $page_data = array();
                $page_data['page_title'] = translate('specialization_details');
                $page_data['id'] = $id;
...
/* End of file specialization.php */
/* Location: ./app/controllers/specialization.php */