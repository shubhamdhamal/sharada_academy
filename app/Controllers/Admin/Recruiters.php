...
class Recruiters extends App_Controller {
    private $module_name = 'recruiters';
    private $table = TBL_RECRUITERS;
    private $module_title;
    function __construct() {
        parent::__construct();
        $this->module_title = translate('recruiters');
    }
    public function index() {
        ...
        $page_data['page_title'] = 'recruiters';
    ...
    public function order() {
        ...
        $page_data['page_js'] = $page_js;
        $page_data['recruiters'] = $this->Crud_model->get_data($this->table, array('is_active'=>'1'), array(), array('order'=>'asc'));
        return admin_view('index',$page_data);
    }
    public function crud(){
        ...
        if ($this->request->isAJAX()) {
            ...
            if($action=='list'){
                ...
                if ( isset($datalist) && ! empty($datalist)) {
                    foreach ($datalist as $list) {
                        $edit = '<a href="'.admin_site_url("recruiters/page/add-edit/".encode_string($list->id)).'" class="btn ripple btn-primary btn-sm mt-1 mb-1 text-sm text-white popup-page" data-placement="top" data-toggle="tooltip" title="'.translate('details').'"> <i class="fa fa-edit"></i> </a>';
                        $details = '<a href="'.admin_site_url("recruiters/page/details/".encode_string($list->id)).'" class="btn ripple btn-secondary btn-sm mt-1 mb-1 text-sm text-white popup-page" data-placement="top" data-toggle="tooltip" title="'.translate('details').'"> <i class="fa fa-eye"></i> </a>';
                        $status = $list->is_active=='1' ? '<span class="badge bg-success cursor-pointer change-status" data-action="change_status" data-id="'.encode_string($list->id).'" data-url="'.admin_site_url('recruiters/crud').'">'.translate('active').'</span>' : '<span class="badge bg-danger cursor-pointer change-status" data-action="change_status" data-id="'.encode_string($list->id).'" data-url="'.admin_site_url('recruiters/crud').'">'.translate('inactive').'</span>';
                        ...
                        $nestedData['actions']      = '<div class="btn-group" recruiters="group"">'.$edit.$details.'</div>';
                        $data[]                     = $nestedData;
                    }
                }
                $json_data = [
                ...
                $validation['is_active'] = ['label' => translate('recruiters'), 'rules' => 'required'];
                ...
                $uis = array('name','url','is_active');
                $up_data = array();
                foreach ($uis as $key) {
                    $up_data[$key] = $this->request->getPost($key);
                }
                $up_data['slug'] = get_unique_slug($this->table,$up_data['name']);
                $up_data['image'] = str_replace(base_url('writable/uploads/'), '', str_replace('http://', 'https://', $this->request->getPost('image')));
            ...
            }else if($action=='update_recruiters'){
                ...
                $id = $this->request->getPost('id');
                $id = decode_string($id);
                $recruiters = $this->Crud_model->get_data_row($this->table,array('id'=>$id));
                if(!empty($recruiters)){
                    $up_data = array();
                    $up_data['is_active'] = '1';
                    if($recruiters->is_active=='1'){
                        $up_data['is_active'] = '0';
                    }
                    $up_data['updated_by'] = json_encode($updated);
                    $up_data['updated_on'] = date(DB_DATETIME_FORMAT);
                ...
                if($this->Crud_model->status_trans()){
                    $new_data = $this->Crud_model->get_data_row($this->table,array('id'=>$id));
                    add_app_log($this->table,'7',$id,$new_data,$recruiters);
                    
                    $json['status'] = true;
                    $json['type'] = 'success';
                    $json['message'] = translate('recruiters_update_successfully');
                }else{
                    $json['message'] = translate('unable_to_update_recruiters');
                }
                ...
                if($this->Crud_model->status_trans()){
                    ...
                    $json['url'] = admin_site_url('recruiters');
                }else{
                    $json['message'] = translate('something_went_wrong');
                }
            }
    ...
    function page($slug='',$id=''){
        if ($this->request->isAJAX()) {
            ...
            if($slug=='add-edit'){
                $page_data = array();
                $page_data['page_title'] = $id!='' ? translate('update_recruiters') : translate('new_recruiters');
                $page_data['page_action'] = $id!='' ? 'update_recruiters' : 'new_recruiters';
                $page_data['id'] = $id;
                echo admin_view($this->module_name.'/popup-add-edit',$page_data);
            }else if($slug=='details'){
                $page_data = array();
                $page_data['page_title'] = translate('recruiters_details');
                $page_data['id'] = $id;
...
/* End of file Recruiters.php */
/* Location: ./app/controllers/Recruiters.php */