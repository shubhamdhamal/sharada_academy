...
class Web extends App_Controller {
    ...
    public function get_new_fms_key() {
    	$return = array();
    	$users = $this->Crud_model->get_data(TBL_ADMIN,array('is_active'=>'1','role_id!='=>'2','web_auth_key!='=>''));
    	if(isset($users) && !empty($users)){ foreach($users as $ukey => $uvalue){
    		$return[] = get_filemanager_key($uvalue->id);
    	} }
